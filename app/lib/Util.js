...
const viewPath = __dirname + '/../sections';
const fs = require('fs');
...
module.exports = (function() {
    return {
        ...
        template: function(template, callback) {
            fs.exists(viewPath + template, function(exists) {
                if (exists) {
                    fs.readFile(viewPath + template, 'utf8', function(error, template) {
                        if (error) {
                            // throw error; // ??
                    ...
                    console.error('Missing file: ' + viewPath + template)
                }
            });
        },
        createWindow: function(options) {
            ...
            win.loadURL(('file://' + viewPath + options.path) || 'about:blank');
