    ...
    var Game = class Game {
        ...
        loadPlugins() {
            ...
            game.plugin.get('game-modifier').forEach(function(plugin) {
                (plugin.contents || []).forEach(function(component) {
                    ...
                    vm.runInContext(code, vm.createContext(game.context), component);
                });
            });
        }
        ...
        new(options) {
            ...
            game.on('turn-end', function() {
                ...
                // last item should always be to kick off the start
                game.emit('turn-start');
            });
            ...
            // TODO: this is testing dadta
            new game.city({
                x: 3,
                y: 4
            });
            game.players[0].setResearch('alphabet');
            game.emit('turn-end');
            game.emit('turn-end');
            game.emit('turn-end');
            game.emit('turn-end');
            // new game.city();
            game.emit('turn-end');
            game.players[0].setResearch('wheel');
            game.emit('turn-end');
            game.emit('turn-end');
        }
