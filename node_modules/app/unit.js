...
module.exports = (function() {
    ...
    game.on('unit-created', function(unit) {
    ...
    game.on('unit-moved', function(unit, from, to) {
    ...
    var Unit = class Unit {
        ...
        applyVisibility() {
            for (var x = this.tile.x - this.visibility; x <= this.tile.x + this.visibility; x++) {
                for (var y = this.tile.y - this.visibility; y <= this.tile.y + this.visibility; y++) {
                    game.emit('tile-seen', game.map.get(x, y), this.player);
                }
            }
        }
        ...
        validateMove(to) {
            ...
            if (movementCost > this.movesLeft) {
                if ((Math.random() * 2) < (this.movesLeft / movementCost)) {
                    this.movesLeft = 0;
        ...
        action(action) {
            if (this.can(action)) {
                ...
                game.emit('unit-moved', this, this.tile, this.tile);
            }
            else {
