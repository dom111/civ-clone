...
Object.entries({
  ...
  'city:building-complete': (cityBuild, item) => `${cityBuild.city().name()} finished building ${item.constructor.name}.`,
  ...
  'goody-hut:discovered': (goodyHut, unit) => `${unit.player().civilization.nation} have discovered a goody hut.`,
  'goody-hut:action-performed': (goodyHut, action) => `${action.unit().player().civilization.nation} have discovered a goody hut (${action.constructor.name}).`,
  // 'engine:initialise': () => '',
  ...
  'player:research': (playerResearch, Advance) => `${playerResearch.player().civilization.nation} have started researching ${Advance.name} (${playerResearch.cost().value()}).`,
  'player:research-complete': (playerResearch, advance) => `${playerResearch.player().civilization.nation} have discovered the secrets of ${advance.constructor.name}.`,
  'player:yield': (playerYield, player) => `${player.civilization.nation} acquires ${playerYield.value()} ${playerYield.constructor.name}.`,
  'tile:improvement-built': (tile, improvement) => `${improvement.constructor.name} built at ${tile.x()}, ${tile.y()} (${tile.terrain().constructor.name}).`,
