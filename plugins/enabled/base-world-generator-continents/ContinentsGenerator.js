...
export class ContinentsGenerator extends BaseGenerator {
  ...
  generateLand() {
    const seen = {},
      ...
      seedX = seedTile % this.width,
      seedY = Math.floor(seedTile / this.width),
      flagAsSeen = (id) => {
        if (! (id in seen)) {
          seen[id] = 0;
        }
    ...
    while (toProcess.length) {
      ...
      if (! seen[currentTile] || seen[currentTile] < this.#maxIterations) {
        const x = currentTile % this.width,
          y = Math.floor(currentTile / this.width),
          distance = Math.hypot(seedX - x, seedY - y)
        ;

        if (
          (Math.random() / distance) > this.#chanceToBecomeLand ||
...
export default ContinentsGenerator;
