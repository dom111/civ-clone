...
for pluginPath in $pluginPaths; do
    dependencies=$(find $pluginPath -type f -name '*.js' -and -not -path '*/tests/*'|xargs grep -R \.js|awk '{print $NF}'|awk -F\' '{print$2}'|grep -P '\.\./[a-z]+-'|sed -e 's!\.\./!!g'|awk -F/ '{print$1}'|sort -u|grep -v core-test|xargs);
